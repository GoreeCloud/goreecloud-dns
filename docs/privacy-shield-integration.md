...
The Infrastructure Status v1 envelope is not the Privacy Shield status contract. Privacy Shield status must remain a separate contract from GoreeCloud Infrastructure Status. Shared runtime evidence may feed both contracts, but their envelopes, environment variables, and state vocabularies are distinct.
