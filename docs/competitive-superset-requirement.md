...
Implemented source foundations now include the first-party Beacon request pipeline, sharded cache, resolver scheduling, classic UDP/TCP transport, iterative recursion, DNSSEC trust-chain validation, authenticated NSEC/NSEC3 denial, wildcard validation, scoped NSEC3 Opt-Out handling, RFC 9824 compact-denial handling, signed CNAME/DNAME alias-chain processing, out-of-bailiwick authoritative nameserver discovery, RFC 9156 QNAME minimisation, forward/conditional/stub and split-horizon resolver routing, runtime self-target protection, routed private DNSSEC trust anchors, private child-delegation DNSSEC trust carry, and migration-evidence/trust-anchor lifecycle work used to gate rehearsal eligibility.

The branch now also contains an initial executable Beacon Policy Profiles foundation through `internal/gcdns/policy_profiles.go`. It implements reusable profiles, exact client-ID assignment, longest-prefix network assignment, a default profile, deterministic rule ordering, exact/suffix/category/service matching against a local first-party catalog, IANA-timezone schedules including overnight windows, allow/block behavior, NXDOMAIN/REFUSED blocks, A/AAAA/CNAME DNS rewrites, and a privacy-minimized decision-recording contract that omits queried names and client identifiers. `docs/policy-profiles.md`, deterministic unit tests, a focused fail-closed source validator, and the native-core CI gate document and protect this source milestone.

Those foundations do not establish feature parity with the full target above. Production-required filtering-list ingestion and integrity, managed category/service catalog content and update provenance, parental-control presets and SafeSearch enforcement, wildcard/regular-expression custom filtering, temporary overrides and profile composition/inheritance, profile CRUD APIs, multi-user/RBAC administration, device enrollment and GoreeCloud Identity binding, encrypted-DNS profile delivery, privacy-aware analytics and retention controls, Glaze UI policy administration, configuration export/import, cluster synchronization, complete authoritative DNS, downstream encrypted listeners and endpoint lifecycle, DHCP, broader administration/API/identity workflows, platform acceptance, exact-artifact runtime evidence, failure/restart testing, performance/security acceptance, backup/restore, rollback proof, and final migration authorization remain required as applicable.
