...
func (s *TargetScheduler) Resolve(ctx context.Context, req *Request) (*Result, error) {
	...
	for _, target := range ordered {
		...
		<-s.sem
		if err == nil && res != nil { return res, nil }
		if err == nil { err = errors.New("goreecloud dns: resolver target returned nil result") }
...
func (s *TargetScheduler) attempt(ctx context.Context, state *targetState, req *Request) (*Result, error) {
	...
	if elapsed > 0 { state.latencyNanos.Add(uint64(elapsed)) }
	if err != nil {
		state.failures.Add(1)
		if errors.Is(err, context.DeadlineExceeded) || errors.Is(attemptCtx.Err(), context.DeadlineExceeded) { state.timeouts.Add(1) }
		return nil, err
	}
	if res == nil {
		state.failures.Add(1)
...
func (s *TargetScheduler) orderedTargets() []*targetState {
	...
	sort.SliceStable(ordered, func(i, j int) bool {
		...
		if aSuccess+aFail == 0 && bSuccess+bFail != 0 { return false }
		if bSuccess+bFail == 0 && aSuccess+aFail != 0 { return true }
...
var _ Resolver = (*TargetScheduler)(nil)
