...
For terminal positive answers, the DNSSEC iterative path groups answer data by owner name and RR type, requires an RRSIG for every positive RRset, and validates each RRset against the authenticated zone keys before returning `DNSSECSecure`. A bogus result fails closed and cannot be inserted into Beacon Cache through the native pipeline.
...
`internal/gcdns/iterative_dnssec_test.go` provides deterministic scheduler/validator tests for root-key acquisition, authenticated parent-key carry across a referral, child DNSKEY acquisition, secure terminal RRset handling, negative-answer fail-closed behavior, and required trust inputs. `scripts/validate_dnssec_iterative.py` adds a focused source-contract validator, and the lint workflow runs it alongside the broader first-party resolver contract. The existing `native-dns-core` job continues to run `go test ./internal/gcdns`.
