...
`internal/gcdns/authenticated_denial.go` now implements conservative authenticated denial for authoritative negative answers. Every NSEC, NSEC3, and included SOA RRset used by the negative response must carry an RRSIG that validates against the authenticated zone key set before its denial semantics are considered. NSEC NODATA requires an exact owner-name NSEC whose bitmap proves the requested type and CNAME are absent. NSEC NXDOMAIN requires proof that the queried name is covered, establishes a closest encloser from authenticated NSEC owner material, and proves absence of the corresponding wildcard name.

NSEC3 denial requires a uniform SHA-1 hash/iteration/salt parameter set. Exact-name NSEC3 NODATA requires a matching NSEC3 owner hash whose bitmap excludes the requested type and CNAME. NSEC3 NXDOMAIN requires a closest-encloser match, coverage of the next-closer name, and coverage of the wildcard at the closest encloser. NSEC3 opt-out denial is not yet supported and fails closed. Ambiguous wildcard-NODATA cases are also intentionally not inferred. This narrow acceptance model is designed to prefer rejection over incorrectly authenticating nonexistence.

Missing DS material at a delegation is still not treated as sufficient evidence of an insecure child. The lower-level DS matching primitive can represent an unsigned boundary, but the iterative trust chain continues to require authenticated denial before an insecure delegation can be trusted. The current authenticated-denial milestone validates terminal NXDOMAIN/NODATA proof material; authenticated DS-absence processing at delegation boundaries remains separate work.

`internal/gcdns/iterative_dnssec_test.go` provides deterministic scheduler/validator tests for root-key acquisition, authenticated parent-key carry across a referral, child DNSKEY acquisition, secure terminal RRset handling, signer-zone key filtering and fail-closed rejection when no authenticated signer key exists, negative-answer rejection when proof material is absent, and required trust inputs. `internal/gcdns/authenticated_denial_test.go` adds deterministic NSEC and NSEC3 NODATA/NXDOMAIN proof tests, unsigned-proof rejection, NSEC3 opt-out fail-closed coverage, and an end-to-end iterative NXDOMAIN path that returns `DNSSECSecure` only after the denial RRsets validate. `scripts/validate_dnssec_iterative.py` and `scripts/validate_resolver_backend.py` both require the authenticated-denial implementation and tests. The existing `native-dns-core` job continues to run `go test ./internal/gcdns`.

This is still not a production-complete DNSSEC resolver. Authenticated DS-absence/insecure-delegation processing, NSEC3 opt-out semantics, wildcard-expanded positive-answer proof validation, wildcard NODATA edge cases, DNSSEC algorithm and key-size policy, trust-anchor persistence and automated rollover state, out-of-bailiwick authoritative name-server address discovery, CNAME/DNAME handling, QNAME minimization, lame-delegation behavior, richer retry/EDNS policy, and target-environment runtime acceptance remain required before production use.
